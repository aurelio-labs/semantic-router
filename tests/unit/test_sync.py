...
def init_index(
    ...
    if index_cls is PineconeIndex:
        ...
        if os.getenv("PINECONE_API_BASE_URL", "").startswith("https://api.pinecone.io") and not os.getenv("PINECONE_INDEX_NAME"):
            pytest.skip("Skipping Pinecone in cloud: set PINECONE_INDEX_NAME to an existing index to run.")
        if index_name:
