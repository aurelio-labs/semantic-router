...
from semantic_router.encoders import (
    LocalEncoder, HuggingFaceEncoder, FastEmbedEncoder, BM25Encoder, TfidfEncoder,
    OpenAIEncoder, CohereEncoder, AzureOpenAIEncoder, BedrockEncoder, GoogleEncoder,
    HFEndpointEncoder, LiteLLMEncoder, MistralEncoder, VoyageEncoder, JinaEncoder, NimEncoder,
    CLIPEncoder, VitEncoder, AurelioSparseEncoder
)
...
ENCODER_MATRIX = [
    ...
    (BedrockEncoder, {"access_key_id": "fake", "secret_access_key": "fake", "region": "us-west-2"}, "boto3"),
    (GoogleEncoder, {"project_id": "fake"}, "google.cloud.aiplatform"),
    (HFEndpointEncoder, {"huggingface_url": "https://api-inference.huggingface.co/models/bert-base-uncased", "huggingface_api_key": "fake"}, "requests"),
    (LiteLLMEncoder, {"name": "openai/text-embedding-3-small", "api_key": "fake"}, "litellm"),
...
def test_encoder_basic(encoder_cls, kwargs, dep):
    ...
    if encoder_cls in [OpenAIEncoder, CohereEncoder, AzureOpenAIEncoder] and not os.getenv("OPENAI_API_KEY") and not kwargs.get("api_key"):
        pytest.skip("OPENAI_API_KEY not set")
    if encoder_cls is GoogleEncoder and not os.getenv("GOOGLE_PROJECT_ID") and not kwargs.get("project_id"):
