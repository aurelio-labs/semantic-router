...
def build_records(
    ...
    routes: list[str],
    utterances: list[str],
    function_schemas: list[dict[str, Any]] | None | None = None,
...
class PineconeLocalIndex(BaseIndex):
    ...
    base_url: Optional[str] = "http://localhost:5080"
    ...
    # def _initialize_async_client(self, api_key: str | None = None):
    ...
    def _init_index(self, force_create: bool = False) -> Any | None:
        ...
        if dimensions_given and not index_exists:
            ...
            if self.index_host and self.base_url:
                ...
                logger.error(f"index_host exists:{self.index_host}")
                logger.error(f"index exists:{index}")
    ...
    async def _init_async_index(self, force_create: bool = False):
        ...
        logger.error(f"index_stats: {index_stats}")
    ...
    def _get_all(self, prefix: str | None = None, include_metadata: bool = False):
        """
    ...
    def delete(self, route_name: str):
        ...
        # index_host = self.client.describe_index(self.index_name).host
    ...
    def query(
        ...
        try:
            ...
            route_names = [
                result["metadata"]["sr_route"] for result in results["matches"]
            ]
    ...
    def __len__(self):
        if self.index is None:
            raise ValueError("Index is None, could not get length.")
