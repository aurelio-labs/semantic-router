...
class BM25Encoder(TfidfEncoder):
    ...
    # _corpus_scores: np.ndarray | None = None
    ...
    def fit(self, routes: List[Route]) -> "BM25Encoder":
        ...
        # idf_num = self.corpus_size - documents_containing_word + 0.5
        # idf_den = documents_containing_word + 0.5
        # idf = np.log(idf_num / idf_den + 1)
        #
        # # Precompute BM25 scores for the corpus
        # # num = tf * (k1 + 1)
        # num = corpus * (self.k1 + 1)
        #
        # # den = tf + k1 * (1 - b + b * doc_len/avg_doc_len)
        # doc_len_norm = self.k1 * (
        #     1 - self.b + self.b * (doc_lengths[:, np.newaxis] / self._avg_doc_len)
        # )
        #
        # corpus_mask = corpus > 0
        # den = corpus + np.where(corpus_mask, doc_len_norm, 0)
        #
        # # Final corpus scores
        # _pre_scores = num / den
        # _pre_scores = np.where(np.isnan(_pre_scores), 0, _pre_scores)
        # self._corpus_scores = idf * _pre_scores
    ...
    def encode_queries(self, queries: list[str]) -> list[SparseEmbedding]:
        ...
        idf_norm = idf / idf.sum(axis=1)[:, np.newaxis]
